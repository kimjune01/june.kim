...
Every payment platform builds a ledger and then builds a protocol to access it. Mailpay inverts this: the protocol *is* the ledger. The emails you already exchanged are the source of truth.
...
Email lets async transactions stay async. The thread accumulates evidence as the work unfolds. The protocol's shape matches the work's shape. When the work is instant, the protocol is two emails. When it spans days, the thread grows to match. Mailpay doesn't have a shape. It has a thread.
...
| Semantic | Agent parsed the payload and accepted the obligation | **Mailpay DELIVER** |
...
| Function | Traditional platform | Mailpay |
|----------|---------------------|---------|
