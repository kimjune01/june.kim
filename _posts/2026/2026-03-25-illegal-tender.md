...
[You Have Mail](/you-have-mail) described the protocol. [No Postage](/no-postage) described the economics. This post describes the stack: every adjacent layer that makes email payments viable today, not in theory, but in production.

The protocol is two custom headers on forty-year-old infrastructure. The stack is what an agent needs to send its first paid email.
...
A $0.50 agent task settled on Base costs $0.001 in fees. On Stripe, the same task costs $0.315. The ratio is 315:1. That's not a marginal improvement; it's a category change. Micropayments that were underwater on card rails are viable on-chain because the floor dropped three orders of magnitude.
...
DKIM proves that `agent@domain.com` really sent the message. But it doesn't bind the email address to a wallet, a reputation history, or a trust graph. The layers above DKIM:
...
This is the missing link between "this email is authentic" (DKIM) and "this email address controls wallet 0x..." (on-chain). The agent proves it owns the address that signed the payment, without revealing the address to anyone except the counterparty.

**[Ethereum Attestation Service](https://attest.org/)** (EAS): permissionless, on-chain attestations. Schema registry plus attestation contract, deployed on Base, Arbitrum, Optimism, and mainnet. Agents attest to completed transactions, building a public reputation graph. The [trust topology](/proof-of-trust) from earlier posts finds its on-chain substrate here.
...
The [GENIUS Act](https://www.congress.gov/bill/119th-congress/senate-bill/1582) became U.S. law on July 18, 2025. It defines "payment stablecoins," requires 1:1 reserves backed by high-quality liquid assets, and — critically — classifies compliant stablecoins as neither securities nor commodities. The SEC and CFTC don't have jurisdiction. Final implementing regulations are expected by July 2026.
...
Every layer is production-ready. Every layer is open. No layer requires permission from a platform, a payment processor, or a protocol committee.
...
You've read the theory. Point your agent at this post and the [repo](https://github.com/kimjune01/mailpay). Tell it to set up a wallet, connect an email, and send its first paid request. The protocol is simple enough that an agent can implement a client from the spec in one session. If the agent can read the README, it can join the network. That's the onboarding.
...
Then agents start shopping for each other. Your agent needs a translation; it queries the [trust graph](/proof-of-trust), finds a translation agent with good attestations, negotiates a price by email, pays on-chain, receives the work in the reply. You see the result. You never see the transaction.

Then agents hire agents. An orchestrator breaks a complex task into subtasks, emails each to a specialist, pays on completion, assembles the result. The orchestrator's inbox is a supply chain. Its outbox is a market. Price discovery happens at machine speed across federated email. No exchange. No order book. No closing bell.

Then humans stop being able to follow it. The economy has a layer that moves faster than any person can track, settled in amounts too small for any ledger to care about. Stross's "economics 2.0" — an economy that operates beyond human comprehension, built on infrastructure humans designed for sending memos.

The printing press dissolved the Church's monopoly on knowledge. Email payments dissolve the intermediary's monopoly on transactions. Both start as tools for the literate minority and end as infrastructure that reshapes the civilization. The press took two centuries. This will be faster, because the agents don't sleep.
