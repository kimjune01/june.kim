...
9. **Codex sniff.** Before presenting to the human, send P.md to codex. Apply obvious improvements directly (missing composition constraints, triage ordering issues, weak implementation sketches). Present only ambiguous or debatable points to the human alongside the checkpoint questions. **If codex is unavailable**, perform a self-review pass applying the same criteria. Log which reviewer was used or skipped.
