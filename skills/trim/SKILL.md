...
3. **Self-recap.** The local twin of #2: a clause that restates its own sentence, or a list re-given in another register a line later (e.g. "true, false, untrue" then "green, red, hung"). The distinction is scale, within or adjacent to a sentence, not across sections. Keep the register that does the most work; cut the recap.
   ...
   This is the highest-yield pattern in a long draft and the easiest to read past, because each sentence is individually fine and the redundancy only exists in the *pair*. Read adjacent sentences as pairs, not one at a time. Four shapes recur:

   - **Definitional tail.** A noun followed by a clause that unpacks what the noun already means. "each match a precedent that *settled the analogous problem elsewhere*" (a precedent by definition happened elsewhere); "managerial accounting is out of scope … is its own business, *unaudited and unstandardized*"; "a *falsifiable* check that would refute it"; "auditors buy it *selectively*, where the expected information per dollar is highest." Cut the tail, keep the noun. Where the tail is the *precise* half and the noun is vague, cut the other way.
   - **Same claim, N phrasings.** One idea stated two or three times across adjacent sentences, each in different words. "the audit layer terminates the chain. An auditor who publishes receipts *closes the door behind them*, since a re-runnable finding *needs no fourth party to vouch for it*." Keep the phrasing that says *how* or carries the citation; cut the rest.
   - **Announce-the-list.** A clause naming the count or shape of a list that immediately follows. "the auditor's second product, *and the job is twofold*. Verify the claim, then …" Overlaps #5 but is subtler, because it hides mid-sentence rather than opening one.
   - **Paragraph-closing echo.** A final sentence that re-says the paragraph in summary register. The most invisible shape, because closers *feel* like craft. Test it against the paragraph's own best sentence; if the closer adds no angle, the paragraph already ended.

   **Two guards.** A deliberate near-echo or callback is cohesion, not recap ("comply immaculately" … "complying in volume") — check whether the repeated words appeared earlier on purpose before cutting. And a colon whose right-hand side turns out to be redundant was propping up filler: cut the clause, do not preserve the colon.
...
5. **Scaffolding / framing moves.** Whole sentences that *announce* a move instead of making it: "The question is X, and the answer is Y," "Having established A, we turn to B," "What is interesting here is." Trim removes the framing sentence; the phrase-level version ("it is worth noting that") is `/tighten`'s. State the conclusion and the frame falls away. **The costliest case is the opener.** The first sentence of any unit, section or paragraph, is its highest-leverage position; spending it on procedure ("Let's name the three," "Consider the following," "Now we turn to") burns that slot on signposting and defers the payload, and the hortatory *"Let's…"* drags in a lecture-hall register besides. Open on a claim or an image, never on an announcement of what the unit will do; let the structure reveal itself in the content (a section titled *The trichotomy* does not need "Let's name the three" — the naming should *happen*, not be promised). Test: delete the opening clause; if the unit reads stronger without it, it was throat-clearing.
...
2. For each section, scan for the patterns and run the absence test on every candidate. On a long document, dispatch a subagent (Agent) to return proposals rather than reading the whole thing inline; pass it this skill's test, the patterns, and the hard-keep list. For a #3 sweep specifically, fan out one agent per section range in parallel, each told to read the whole file for context but propose only inside its range, and each returning exact unique substrings plus replacements so the orchestrator applies them centrally. Agents that edit in parallel collide; agents that propose do not. Give them two or three *concrete* before/after pairs from the document at hand, since the pattern is far easier to recognize from an example than from its description.
