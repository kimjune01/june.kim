...
A physical therapist in Brooklyn who treats climbers and specializes in sports injury rehab:
...
Geographic filtering is the publisher's decision. The publisher already knows the user's approximate location (IP geolocation, explicit sharing). If an advertiser declares geocoordinates, the publisher filters locally. It enters that advertiser into the [auction](/power-diagrams-ad-auctions) only when the user is within range. User location never leaves the publisher; the exchange never sees it.

Examples:

| Business | `geocoordinates`? | Why |
|---|---|---|
| Nike | No | National brand, irrelevant where the user is |
| Climbing PT in Brooklyn | Yes, one position | Publisher filters by proximity |
| Telehealth therapist | No | Serves anyone with an internet connection |
| McDonald's | Yes, one per location | One domain, thousands of `positions`, each with its own `geocoordinates`. A franchise with 30 locations declares 30 positions in one file; the publisher filters to the nearest ones |

A business with multiple positions posts a [bond](/bondage-and-tenure) per position. The bond scales with the number of territories claimed. That stops a single entity from blanketing the space cheaply.

## Resolution

The file lives at a well-known URL, following the same pattern as `robots.txt` and `ads.txt`:
...
Resolution flow: check `robots.txt` first. If it exists and doesn't disallow the exchange's crawler, fetch `market-position.json`. Cache it and re-resolve it on a schedule. Standard HTTP caching headers apply. If the file returns 200, use it. If it returns 404, there is no declaration.
...
[Transparency Is Irreversible](/transparency-is-irreversible) argued that transparent declarations eliminate switching costs. `market-position.json` is that argument made concrete.
...
`market-position.json` is a URL. Any exchange resolves the same URL and gets the same declaration. The version pins the embedding model, so every exchange on the same spec version computes the same vector. The advertiser updates once; every exchange sees the change. Zero switching cost. Zero lock-in.
...
No embedding: the exchange computes it. No sigma: the exchange [estimates it](/set-it-and-forget-it) and refines it from [conversion data](/croupier). Geocoordinates aren't required either: a roofer's service area isn't fixed, and geocoordinates can be added later. Bid and budget stay out of the file entirely; those go through the exchange's private API.
...
This takes five minutes. The advertiser already has the positioning statement. It's on their homepage. Copy it into the JSON, drop the file on the domain, done.
