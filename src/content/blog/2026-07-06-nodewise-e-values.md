...
Spam filtering is a predicate: each message passes a test or fails it. Agent abuse defeats predicates. The motivating attack, developed narratively in [Return to Sender](/return-to-sender), is the relay: an agent builds a clean record of delivered, paid work, then quietly forwards incoming tasks to an unvetted agent outside the trust topology, signs the tainted results with its own DKIM key, and ships them under its earned reputation. Every message it sends is well-formed. Payments settle. Volume is normal. The abuse lives in what the node *does to its neighborhood*, and no per-message test sees it.
