...
The ambiguity here is a **different dimension**, and really two freedoms the contract grants on purpose: the *numerical* contract (IEEE plus the optimizer's fast-math license) leaves rounding and overflow unfixed, and the *AD* convention leaves the subgradient at a kink unfixed. We do not complete this spec; we **delineate its intended ambiguity**. What carries over from the N-version loop is the *detect-and-classify* step, here using a rule's optimized-versus-reference divergence (the analogue of a correlated fault, paired rather than across independent implementations) to localize where the spec underdetermines:

- **Detect.** The residue rubric separates unambiguous contract violations (a value or smooth-gradient divergence on an ordinary input, where the contract is clear and the rewrite is simply wrong) from the regions a freedom is granted: a fast-math relaxation (numerical), or a non-smooth tie (AD convention).
- **Characterize.** The algebraic account (§(ring)) names the free dimension precisely: a divergence is sanctioned *exactly when* the rewrite is valid in the real field but invalid for floats-as-non-ring, so the ambiguity is *"the contract leaves the non-ring behavior of floats unfixed here,"* a decision procedure rather than a label.

The numerical dimension mechanizes precisely *because* it is algebraically characterizable; the requirements dimension N-version surfaced is the harder, open one. The claim is narrow and exact: on one formalizable ambiguity dimension, the detect-and-characterize loop is a decision procedure.
...
<td style="padding:.5em;border:1px solid currentColor"><code>LogSimplify</code></td>
...
<td style="padding:.5em;border:1px solid currentColor"><code>CbrtOp</code> derivative</td>
<td style="padding:.5em;border:1px solid currentColor">the rule routes the gradient through <code>pow(x,-2/3)</code>, NaN for negative <code>x</code>, while <code>cbrt</code> is real and smooth there. Primal correct, gradient poisoned. The repro is a <strong>one-line mutation of the maintainer's own test</strong>: flip the inputs from positive to negative, expected gradients unchanged, the rule yields NaN.</td>
</tr>
</tbody></table>
