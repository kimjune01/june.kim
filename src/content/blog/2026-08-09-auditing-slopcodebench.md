...
- Running all 196 answer keys through the benchmark's own harness, in the benchmark's own Docker image, finds 4 deterministically defective (`dynamic_buffer` checkpoints 2 through 4, `env_manager` checkpoint 3), 1 unstable (`pwd_manager` checkpoint 4 fails a different regression test on each run), and 8 that grade TypeScript through an unpinned `npx` toolchain that resolves to a broken version today. [Gold sweep receipt.](https://github.com/kimjune01/slopcodebench-audit/blob/main/findings/06-gold-sweep.md)
