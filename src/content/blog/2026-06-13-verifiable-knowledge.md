...
Large language model (LLM)-based agents cannot be held accountable. Their reasoning disappears with the context window, and even full provenance trails only attest that work happened, never that a belief earned its entitlement. The burden of proof falls on whoever drives the agent. This paper introduces **verifiable knowledge**, a protocol in which each agent, instead of *attesting* its own work, presents each claim with a falsifiable condition another agent can re-run, reaching the same verdict where the check pins its roots and a cross-checked one where it cannot. That replayable claim is the **verification primitive**: belief, knowledge, and truth reduce to structures one agent constructs and another checks, with entitlement conferred by re-running the check rather than by the author's word or credential. Replay breaks the who-trusts-the-truster regress that attestation spawns, and verifiable claims compose, so a population that agrees on the method rather than on the beliefs accumulates a shared **canon** without a gatekeeper, every entry re-checkable by any member and revocable by a failed replay. Accountable failure outranks unaccountable assertion. The epistemics is borrowed from *What Cannot Be False Cannot Be True*; the contribution is the contract that runs it across machine and social boundaries. Adopting it makes agents more accountable by construction; the one falsifiable bet is that it also leaves them better coordinated than agents that do not.
