...
For another agent to check rather than merely disagree, the projections have to compose, and that is what checkability buys. The causal order runs from replayable contact to independent convergence, and only then to an inferred shared object: the best surviving hypothesis about what constrained them. A replayable check lets one agent re-run another's touch, while convergent projections reinforce that they touched one object, though no agent sees it whole. Constraint thus becomes intersubjective convergence at the replay, and the shared check closes the gap to a shared verdict.
