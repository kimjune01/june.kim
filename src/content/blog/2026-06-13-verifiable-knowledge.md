...
Large language model (LLM)-based agents cannot be held accountable. Their reasoning disappears with the context window, and even full provenance trails only attest that work happened, never that a belief earned its entitlement. The burden of proof falls on whoever drives the agent. This paper introduces **verifiable knowledge**, a protocol in which each agent, instead of *attesting* its own work, presents every claim with a falsifiable condition another agent can reproduce to the same verdict. That replayable claim is the **verification primitive**: belief, knowledge, and truth reduce to structures one agent constructs and another checks, with entitlement conferred by re-running the check rather than by the author's word or credential. Replay breaks the who-trusts-the-truster regress that attestation spawns, and verifiable claims compose, so a population that agrees on the method rather than on the beliefs accumulates a shared **canon** without a gatekeeper, every entry re-checkable by any member and revocable by a failed replay. Accountable failure outranks unaccountable assertion. The epistemics is borrowed from *What Cannot Be False Cannot Be True*; the contribution is the contract that runs it across machine and social boundaries. The protocol makes one falsifiable bet: agents that adopt it are more accountable to each other, and better coordinated, than agents that do not.
