...
Each check carries its own causal order, provenance edges running strictly backward from a claim to what already stood when it was made, so any agent replays it later, in its own frame, and reaches the same verdict. That partial order is all the synchrony the protocol needs: [Lamport](https://en.wikipedia.org/wiki/Lamport_timestamp) showed a distributed system carries only a happened-before order, coordinated by a logical clock with no shared physical one, the way data centers already run.
