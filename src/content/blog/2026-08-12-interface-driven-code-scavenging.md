...
## Read the repair, not just the patch

An inspiring commit shows *what* changed before reality has graded it. Before adapting it, follow the history backward to the motivating issue and forward through the introducing PR, later fixes, and current implementation. With [Creusot's extern-spec documentation](https://github.com/creusot-rs/creusot/pull/1851), the first diff supplied the `cfg(doc)` mechanism; the [issue](https://github.com/creusot-rs/creusot/issues/1104) revealed that searchable surrogate pages were a compromise, and a [later repair](https://github.com/creusot-rs/creusot/pull/1963) exposed link failures that CI did not enforce. The commit says *what*; the repair lineage says *why this shape, where it broke, and what now keeps it true*.
...
- Huacan Wang, Ziyi Ni, et al., [“RepoMaster: Autonomous Exploration and Understanding of GitHub Repositories for Complex Task Solving”](https://arxiv.org/abs/2505.21577) (2025). An agent that searches GitHub, maps a found repository’s call and dependency graphs, and drives it to solve the task, lifting GitTaskBench’s pass rate from 40.7% to 62.9% over the strongest generate-from-scratch baseline. It stops at execution; the repository remains a tool. Scavenging carries the code across the ownership boundary.
