...
Without the lenses: "hiring is broken, fire the VP." With the framework: level one. With the handshake: level two. With the parts bin: three. With the taxonomy ordered: four. You can't name the grid coordinates without the contracts, can't name the contracts without the steps, can't name the steps without the loop.
...
1. **Deny.** "Our complexity is structural, not diagnostic." The test: can you name the broken contract in one sentence? If not, it's diagnostic fog.
2. **Adopt.** Learn the taxonomy. The incumbent's advantage (headcount, data, distribution) now competes against diagnostic speed. The moat becomes a race.
3. **Acquire.** Buy the diagnostic capability. The taxonomy is the asset.

The deepest moat is the one that's hard to diagnose even *with* the taxonomy. Turbulence is a real moat. "We have a lot of engineers" is not.
