...
Technical hiring is a mess of confusion, and the mess starts at the screen. Resumes get grepped for tech-stack nouns; recruiters pattern-match on school and employer names. Nouns and names are proxies, a limitation widely recognized but rarely measured. The whole apparatus runs on hope: that keyword density and credential prestige correlate with what anyone actually wants. If we want to do this methodically instead, what are we supposed to measure anyway?
...
The pattern: every folk remedy that works, works by getting closer to observing the candidate doing the job. None scales cleanly, their fairness varies, and the mechanism is rarely measured, so they are difficult to improve and easy to repeat ritually.
...
And this is the industry that A/B tests button colors. The data-driven era instrumented everything that was cheap to instrument; hiring, the most expensive recurring decision in the building, stayed largely unvalidated, even at the company that made data-driven a religion.

## Known and still used

The evals world ran the same tautology in public. Contamination concerns around SWE-bench Verified, the standard benchmark for AI coding ability, were widely known. It kept getting reported in model cards and launch posts because it remained the shared number. It took until February 2026 for OpenAI to [deprecate it](https://openai.com/index/why-we-no-longer-evaluate-swe-bench-verified/) as saturated and highly contaminated. A model reproduced the exact gold patch from a short problem snippet plus the task ID, and every frontier model tested showed task-specific contamination. The candidate had memorized the answer key, and the interviewers kept asking the questions for two more years, because those were still the standard questions.
...
*Nobody gets fired for hiring IBM, until someone does.* Running the standard loop is defensible rather than valid. Defensibility is a social property. It holds until the day it doesn't, with little warning, because the measurement that could provide one is rarely done. And the market can't correct what it can't attribute: slow hiring and regretted attrition are seldom traced back to the instrument, so the loop can survive the bad outcomes it causes.

## What a good bench has to have

One property above the others, then four in tension. The typical interview loop compromises all five.
...
- *Discriminating power.* Scores must spread the population; a bench with a compressed score distribution measures little. LeetCode is heavily gamed now: the prep industry compressed the distribution until it increasingly separates the prepped from the unprepped, which was never the construct.
...
- *Efficiency.* Signal per dollar. Onsites burn five engineer-hours per candidate for a verdict with substantial interview-to-interview variance. Work trials get closer to the construct but cost more, a tradeoff evals people price deliberately and hiring often prices by folklore.
...
The thesis: hiring is a benchmark evaluation problem, and many pathologies found in AI benchmarks also appear in interviews. Benchmark people have names and fixes for them; software hiring seldom uses that vocabulary. The crosswalk:
...
<tr><td>Rejected great candidates</td><td>False negatives rarely measured</td></tr>
</table>
...
What the objection misses: IO psych measured and published, but adoption remained sparse, because published validity coefficients don't make rigor *defensible*. Psychometricians aren't the employers the industry copies. "Implement whatever Bock recommends in the literature" has been a strong available move for a decade, and relatively few play it. The antagonist here isn't a missing science. It's the implementation gap between what selection science established and what software companies do, and the agent era just made that gap newly expensive.

## The bar decays

The gap persists because changing a familiar bar is institutionally riskier than retaining one that used to work. And it likely did work: the loop that found great people in 2015 was measuring something, before the prep industry absorbed it. This is benchmark decay, with the prep-course industry as the training-data pipeline. Validity expires while the number keeps getting reported. But benchmarks at least get versioned and retired. Interview loops rarely do. What reads as prudence can become faithfulness to a measurement whose expiry date passed unnoticed, because its validity was not being rechecked.

And many who do change the bar still do not treat hiring as a measurement device. They redesign on taste: whatever the founder hated about their last job's loop, whatever's fashionable this year. No whiteboards! Pair programming! Take-homes! No take-homes! Each redesign risks swapping one unvalidated instrument for another, and the debate runs on vibes when nobody has stated what the instrument is supposed to measure. The field oscillates between a stale measurement and an unspecified one, and calls the oscillation progress.

## The system you hire

This year I applied to two of the organizations that professionalized AI evaluation. METR's coding screen came with a rule: ninety timed minutes, "no AI assistance allowed," for a role at the org whose job is measuring what AI can do. I declined and said why: I can't code without AI assistance anymore. Epoch AI's screen was for a role titled Researcher, Benchmark Reviews. It asked for benchmark-validity critiques written without LLMs, internet encouraged, and warned that "if we find evidence an applicant has used an LLM to answer these questions, we may choose not to review that application." The questions were about instrument failure. Two prominent eval organizations, needing to evaluate me, reached for the same move as much of the industry: ban the tool the job runs on, and grade the ban by judgment call. The generous read is also the correct one: a cheatproof item for a tools-allowed candidate didn't exist for them to reach for.
...
- *Item statistics.* Track which questions change decisions and which produce the same verdict for everyone. Kill the dead items. Benches do this arithmetic routinely; hiring loops rarely do.
...
Until Google, OpenAI, or Anthropic publishes its hiring best practices for technical talent, the industry is unlikely to move at scale. The driving force is economic: verification keeps getting cheaper [relative to trust](/trust-is-a-cache), and change arrives wherever the curves cross. For a century, actually measuring a candidate often cost more than trusting the proxies, so the proxies won. The agent era shifted the ledger on both sides at once. The bench side of verifying got cheap: floor calibration runs without a single candidate, item drafts are model-cheap, an answer-key screen needs no expert judge, re-versioning is automated. The trust signals weakened: a resume, a portfolio, a take-home are now cheap to fake. What stayed expensive was never the blocker: the live session, the graders, the validation. Orgs already pay five engineer-hours per candidate for noise. The crossover is that the same spend can now buy a calibrated instrument. That is what drives the change.

Prestige only sets the timing. Hiring adopts a new *defensible* default when a widely copied employer mints one. The frontier labs are especially well positioned: the field that professionalized evals, and employers whose loops the industry already copies, leaked and secondhand via prep courses. Publishing would replace the leaked copy with the calibrated original.
