...
*Position / survey draft. Agents that act in a world increasingly write their own skills, growing a library of reusable abstractions and planning over it. The hard part is not proposing a skill but deciding which to retain, and that retention criterion forces a choice between generality and specialization. It is an eviction problem, the oldest one in systems: a bounded store read far more than written, where you keep either what is used most or what is dear to recompute. Two communities that rarely cite each other answer with exactly these, compression (description length), which keeps what recurs, and planning utility (Minton's macro utility problem), which keeps what pays off. We make them directly comparable. They are *empirically near-equivalent wherever reuse tracks search-value*, the regime standard Blocksworld occupies, where either keeps the same library and each field succeeds with its own. They diverge most sharply on *rare-but-critical abstractions*, the specialists compression cannot see: the regime we conjecture long-horizon world-model agents inhabit, where today's ever-growing skill libraries weigh nothing against carrying cost. The contribution is the map of where the two criteria agree and the corner where they part.*

## Introduction

Agents now write their own skills. Give a language model an environment and it will propose, code, and stash reusable behaviors, growing a skill library it plans over; Voyager is the canonical case. This read as a glimpse of recursive self-improvement: an agent that accumulates its own capabilities should compound them. It has not been that simple. A library that only grows is not a library that improves. Past some size, more skills mean more to match, retrieve, and confuse, and the agent slows under the weight of what it has learned. The hard part was never proposing a skill, which a language model does fluently, but deciding which to retain. This retention criterion is the organizing question here.

The choice it forces is between generality and specialization, and it is older and wider than language-model agents: HTN learners, program-induction systems, and hierarchical reinforcement learners all grow and prune the same kind of library under other names. The mechanism is the oldest one in systems. A skill library is a bounded store read far more often than written: at every planning step the agent matches its situation against the library, and a larger library speeds the cases it covers while taxing every match. Deciding which entries to keep is cache eviction, and the two answers are the two classic eviction policies. One keeps what is used most, which compression formalizes as keeping what shortens the record of past behavior, the general, broadly reusable skill. The other keeps what is dear to recompute, the rare specialist that cracks one hard case and is invisible to frequency, which planning utility formalizes as keeping what most reduces future search. These two criteria, compression (minimum description length) and planning utility (Minton's macro utility problem), recur across literatures that rarely cite each other; the contribution is the map of where they agree and where they part.

This is an opinionated map, not a systematic review. It draws on five lineages, macro-operator and explanation-based learning, grammar induction, HTN-method learning, hierarchical-RL option discovery, and program-library and LLM skill-library learning, and reads them through one lens: the propose-then-keep decomposition. Where it surveys, it credits originators; where it argues a position, that the retention criterion is load-bearing and that long-horizon agents will need a utility one, it says so, and the advocacy stays visible and separable from the map.
