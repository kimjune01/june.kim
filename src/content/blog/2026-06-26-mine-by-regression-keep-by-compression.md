...
They price different things: representational length, posterior probability, or expected use-time value. They are best read as three ways of pricing an abstraction against what it costs to carry, but only the utility criterion prices *use-time value* directly; compression and the Bayesian prior price encoding length and bet that it tracks value. The library is useful only insofar as it changes planning over `M`.
