...
The reviewer opens the diff: one commit, a link to a [pre-registered](https://en.wikipedia.org/wiki/Preregistration_(science)) experiment, eighty tests matching the prototype's assertions. They review the code, not the premise. No back-and-forth about whether the algorithm works — that question has a [URL](https://github.com/kimjune01/union-find-compaction) with seven trials and a p-value. The bottleneck in a collaborative repo is rarely the build. It's the review — a maintainer deciding whether to trust your contribution. Prework converts "trust me" into "here's the provenance." What lands in production is just the receipt.
