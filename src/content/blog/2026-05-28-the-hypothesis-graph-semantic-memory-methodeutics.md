...
### Cost and replication {#cost-envelope}

~$5.14 per instance for the frontier pair at public API rates (~$3.7k across Pro; observed cash ~$870 under subscription subsidy), ~$0.41 for the open-weight pair, four small EC2 boxes, no GPU. The per-leg derivation and the cash-versus-economic reconciliation are in `COST_BASIS.md`. A consumer subscription plus patience, or a low-four-figure budget, replicates either run.
...
We do not attest; the receipts do. No method documented in our comparative search (§(search)) has demonstrated, with equivalent receipts, a higher SWE-bench Pro official-grader resolve rate at a lower audited per-instance cost under one frozen harness. That claim is about the artifact and its auditability, and it survives the attribution below. What does not survive is any reading of the rate as evidence for the method.
...
The first version of this paper asked what earns the gap and answered honestly that it could not decompose it. The completed program can. Each cut is preregistered or receipt-committed in the repository, including one retracted estimate and its worklog trail; the paper keeps the findings.
