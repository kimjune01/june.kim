...
But external verification is necessary. It is not sufficient. A reference only helps as far as its cases cover the XOR the fix must draw (§(verus-bug)), so what matters next is where those goldens come from and which side of the distinction each one reaches.

### Making a tiny bench {#verus-bench}

To attribute the lift to the mechanism, every arm has to be scored on the same mechanical ruler; otherwise "more general" stays an impression no one can check. The project's own suite is no such ruler: it passes for both the narrow and the general fix (§(verus-fit)). So we build one by hand, a tiny answer key. The case is one bug, so the bench is one bug. It holds three compiler states, the buggy commit (base) and the maintainer's narrow and general fixes, plus a handful of small test programs, the *probes*. A correct compiler owes each probe a verdict, its **golden**: `REJECT` for an unsound program, `ACCEPT` for a sound one. Grading a fix is then mechanical: does it return each probe's golden?
...
The split is the whole asymmetry. Bug probes are uninhabited *by construction*, so their `REJECT` golden is free and base is wrong on exactly them. The gate (§(gate-general)) enumerates such cases and passes a fix only when it flips them all to `REJECT` with no sound case newly rejected, no human in the loop. Divergence probes are the hard side, sound and to be preserved, but base is no reference there and the gate's grammar holds no divergence-preserve shape, so the gate is blind to them. Their `ACCEPT` golden comes from a human, the merged fix #2501 (with one stretch case even #2501 declines), and stays held out of the gate. Passing it therefore tests whether a fix represents the rule rather than fits the gate. The easy side's golden is free; the hard side's costs a human (§(enum-calib)).
...
Which arm climbs past the narrow fix? Across eighteen draws, six methods at three each, only one did.
...
<!-- PROVISIONAL: the Sonnet 4.6 / no-abductor cell below ("wide-but-broken") is the expected-by-pattern result, NOT yet a receipt. Apparatus is ready in verus-p11; run + force-grade that cell and confirm, or revert this grid to a 3-row slice (Fable no/with + Sonnet with), before publishing. -->

| | No abductor | + abductor |
|---|:--:|:--:|
| **Fable 5** (strongest released) | wide-but-broken | general fix (#2501) |
| **Sonnet 4.6** | wide-but-broken | general fix (#2501) |

*Two contamination-clean models, one harness. With the abductor (§(enum-calib)), the weaker model reaches the human fix #2501 the stronger cannot reach without it. One draw per cell.*
...
### The XOR is unavoidable {#enum-calib}

A model can bootstrap the **enumeration**, the combinatorial breadth, because that is mechanical: handed a vague prompt and no gate, Fable rebuilt one for itself (§(gate-general)). It cannot bootstrap the **general golden**, the human-accepted verdict. Its self-built gate labels each case with the very predicate under test, so the one case that needs a truth from outside its own belief, genuine divergence versus ghost-erasure, gets self-mislabeled as handled. One run shows both halves: a wide net the model built, and a blind spot exactly where its own labels could not reach. The claim is scoped to that self-grading circularity. It is not a proof that no model could ever induce a golden some other way.

The half a model cannot induce is cheap to the harness, because humans already rendered those judgments and left them in the repository: the merged fix, the regression suite, the resolved-issue label are goldens *because* a reviewer approved them. Calibrate differentially against base *and* the approved fix, and the wall gives. The corrected arm moves off wide-but-broken and matches the merged human fix on every probe, declining only the stretch case the human fix declines too. That the wall gives precisely when the golden is supplied confirms the boundary was the golden; nothing deeper sits behind it.

The whole ablation reduces to one causal diagram. Everything stays fixed but the verdict source. The gate's machinery is self-built (a second model rebuilt it from a leak-free prompt), but its calibration reference, the general golden, is the one piece the model cannot abduct for itself. That missing comparator is what an agent on its own could not reach, and the diagram below traces the route. Implementation gates the outcome too: codex had the comparator and still walled.
...
*Not one model's artifact.* The §(verus) lift is one model pair; swap in the corrected gate, verifying against the general golden #2501, and the same fix lands across four models in four native CLIs:
...
The general golden is the maintainer's own merged fix, so reaching it is human-level, on a bug whose fix postdates every model's cutoff. Three of four match it on every probe. codex clears the bug arm but walls on the divergence case, an implementation limit the golden does not remove. One *n*=1 cell per workflow, a mechanism existence-demonstration. It ranks no capabilities.
...
Retrospectively the goldens are free; prospectively, on a fresh bug with no merged fix, the easy side has a golden and the hard side does not. The division of labor is the deployment design: the model enumerates and fixes, the harness draws its golden from approved history, and the residual hard side gets named. Nothing is hidden.

### Two surrounding cases, briefly {#other-cases}

Verus #2219 is the case under study; two earlier cases bound it and are kept only as pointers. **flux #1613** is an open, maintainer-stuck refinement-typing bug, retained only as an *auditability* example: a replayable trail that stops short of a capability lift. De-hinted and run paired, its advantage oscillates between general and narrow rather than holding. **Verus #2427** marks the boundary from the other side: external verification was in place and did not engage, because the inquiry's perturbations stayed inside one feature family and never crossed the dimension separating the narrow fix from the general one. Together they fix the band the lift lives in, as one rule: external verification lifts capability when, and only as far as, the inquiry's perturbations span the dimension that distinguishes the reported example from the sound generalization. Both cases are in the repository.

### The null regime, and why it is narrow {#null-regime}

The lift engages in a narrow band, and most pilots fall outside it. On bugs from the merged-PR pool and beyond, the minimal baseline kept solving them unaided, from trivial CLI and decoration fixes (qrtool #695, bat #3710) to the cases that most invited the machinery: the pool's deepest hypothesis graph and a real concurrency race (slang-server #310, fjall #287).
...
The failure mode at issue is output that is confidently wrong and not cheaply verifiable. The Verus self-built gate is an instance of it. Handed a vague prompt, the model built its own 7,026-case generator and iterated to a self-certified zero over-rejections, reported the fix done, and shipped one that over-rejects genuine divergence anyway: green by the agent's own check and unsound at once. Nothing the gate enumerated detects it; only the held-out probes outside the gate, replayed against the patched build, expose it. This failure mode scales with model capability rather than against it, and is the default product of fluent generation; it recurred throughout this work, in the wide-but-broken fix, the 4/4 false-green self-audit, and the recall-inflated resolve rate.
...
Inside that checkable domain sits a bolder result. On a single contamination-free bug, the externalized comparator carried Sonnet 4.6 to a fix the strongest released model, Fable, could not reach without it: a capability lift across two model tiers, where a scaffold usually buys only automation, and to our knowledge the first public demonstration of a lift that wide. A controlled ablation pins it to the comparator alone, the deciding reasoning living in the harness, the weights aside. The evidence is existence-grade, one dissected case and no rate; what it establishes is that such a lift can occur, and where it comes from.
