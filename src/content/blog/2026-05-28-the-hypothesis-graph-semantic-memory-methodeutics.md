...
### Attributed nulls and the typing protocol {#null-regime}
...
The pilots are the case in point: the lift engages in a narrow band, and most fall outside it. A minimal baseline solved them unaided, from trivial fixes (qrtool #695, bat #3710) to the cases that most invited the machinery, a deep hypothesis graph and a real concurrency race (slang-server #310, fjall #287). Two findings attribute the nulls. A **selection artifact**: the deployment pipeline's own triage fast-paths easy bugs around the graph ("a 1-line fix with a confirmed reproducer doesn't need a hypothesis graph"), so the merged pool is exactly the subset where the graph is unneeded. And **baseline reach**: a capable model in a minimal loop fixes most reproducible bugs unaided, so the band is bounded below by that reach and above by triage. The 81 merged PRs inherit this, never graph-dependent, which is why a population rate over an unselected pool measures the band's width and misses the mechanism.
...
Does the Peircean framing do real work, or is it decoration? On this bench, as *rhetoric* it is decoration (§(right-regime)): the prompt ablation measured whether the vocabulary makes a single diagnosis smarter, and it does not. As *protocol* it is essential, which resolves the null rather than sitting beside it: a protocol does nothing on any single instance, the way a wire format makes no message smarter, so a per-instance ablation against loose prose *should* read null, and did. The value accrues across instances. A hypothesis graph written in one context window is read in another, by a different model, a parallel agent, or a human auditor, and the mode labels make a node interpretable without the context that birthed it: abduction marks a belief proposed-untested, induction test-backed, the credence cap traveling with the type. So accountability composes: agent B reruns agent A's kill, agent C builds on B without re-running A, an auditor enters at any link, the warrant flowing down with the receipts (§(epistemics)'s per-hop transfer, across the population). Loose vocabulary caps accountability at one hop and dies with its author's window; that is what 385 graphs in one vocabulary are for, and the precondition for everything §(future-work) builds on.
...
**Persistence beyond the context window.** Chain-of-thought in the context window is lost when it closes; the hypothesis graph externalizes the reasoning first, demonstrated when a build host failed mid-investigation and only the graph and patch survived to resume. The externalized form is *re-runnable*, not merely readable, provided each node carries its own reconstitution. Its scope is bounded: the logical content has no half-life, but the runnable form inherits its apparatus's, replaying only while flux builds, z3 runs, and the SHAs resolve. What it preserves is *checkability* rather than truth, so a false claim backed by provenance stays detectable where one backed by trust is laundered as its witnesses disappear.
...
**Accountability, in deployment.** The intro's promise was a claim that clears only when an independent party reruns its recorded trial (§(introduction)); the receipts deliver it. To a maintainer the agent and its operator are strangers, and with the trial attached that stops mattering: a red-on-master/green-with-fix test and a clean suite read identically whoever submitted them. The 81 merged PRs are the ecological evidence, agent-selected issues and agent-authored patches that adversarial maintainers accepted at 50.6% on the attached receipts (§(availability)), the review bottleneck answered by making the check cheaper than reconstructing the reasoning.
