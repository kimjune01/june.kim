...
## The mechanism by case study: Verus #2219 {#right-regime}

We want to know whether the harness actually changes how a model fixes a real bug, and if it does, which part of it does the work. We first tried to measure that on an established coding benchmark, and learned that such benchmarks aren't set up to measure discovery.

### Coding benchmarks measure translation ability {#bench-translation}

A SWE-bench-shaped coding benchmark hands the solver three things: the issue text, the repository at the buggy commit, and a hidden test the fix must turn from red to green. The one expensive step, deciding what is actually wrong and what the correct general fix is, comes handed over inside the problem.

SWE-bench Pro ([Deng et al. 2025](https://arxiv.org/abs/2509.16941)) is the dominant such benchmark, the one OpenAI now recommends over Verified, and its tasks split into two halves that each leave a diagnostic method nothing to do. The well-specified tasks are one-shot from the prompt for a competent model, so there is no cause to discover. The underspecified ones cannot be discovered at all: what the hidden test grades is the author's unstated intent, absent from both the issue and the repository, so no inquiry recovers it short of reading the held-out test. Our determinacy audit of all 728 public tasks measures the second half, and a small oracle-free probe agrees: handing the model a diagnosis changed nothing, and even perfect localization still lost on an implementation detail the prose never states ([audit](https://github.com/kimjune01/swebench-pro-audit)).
...
With the failing tests visible, the harness resolves 95.3% of SWE-bench Pro's public split under the official grader ([bench run](https://github.com/kimjune01/swebench-pro)): given the oracle, the translation step is largely solved. So the interesting work is now discovery, an undiagnosed bug whose correct fix is a predicate, not a case, with no cheap oracle and a genuinely hidden cause. Far from a corner case, that regime is most of software, every issue tracker a backlog of undiagnosed problems waiting on the one expensive step a benchmark never exercises. We went looking for a bug where discovery is the whole difficulty. Verus #2219 is one: its shipped test is passed by the maintainer's narrow fix (#2230), so the translation is free, while the correct general fix is pinned by no test and goes undiscovered even when the prompt demands it outright. The mechanism experiment ([github.com/kimjune01/hygraph-mechanism](https://github.com/kimjune01/hygraph-mechanism)) dissects it end to end under control, running the contrast a benchmark cannot, on a single case rather than a population rate. Selecting the case for the property a benchmark grades away is the experiment, not a thumb on the scale.
