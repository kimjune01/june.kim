...
<figure>
  <img src="/assets/hypothesis-graph-anatomy.svg" alt="A hypothesis graph as a two-node linked list. The first node, an abduction (the cache is stale), records its trial as an exact command ($ clear cache; rerun), its outcome (still fails), and a credence of 50 percent capped by its mode; it is killed. An edge generated by that kill (generate-edge-from-kill) names the next node. The second node, an induction (the index is missing), records its trial ($ add index; rerun), its outcome (tests pass), and a credence of 96 percent that is test-backed; it is witnessed. The footer states the replay invariant: every node rebuilds from its recorded trial, by a reader who need not trust the author." style="max-width:600px; width:100%; height:auto; margin:1em auto; display:block;" />
  <figcaption><strong>Figure.</strong> A hypothesis graph, two nodes and the edge between them. Each node binds a hypothesis to a trial stated as an exact command, an observed outcome, and a credence capped by the mode that earned it: abduction proposes and stays low, induction is test-backed and rises. The kill on the first node generates the edge to the second. Every node rebuilds from its recorded trial, so a reader replays the structure instead of trusting it.</figcaption>
</figure>
...
### The number, with receipts {#tables}
...
If the gate substitutes for the probe inside diagnosis, does it substitute for the whole stage? Delete the stage: a minimal prompt with no graph and no typed inquiry, handed only the raw problem statement and the failing tests, is the no-diagnosis arm. Determined causes: Δ = +0.000 (n=18). Underdetermined: Δ = +0.062 (n=16), the entire effect one instance, the two clear divergences splitting in opposite directions (no diagnosis beat a wrong one on one instance and lost to a good one on another). Reweighted to the population, that minimal prompt lands near **94%**, about a point under the full harness. The minimal prompt reaching the number is the attribution made plain: on this bench the smem is redundant on the bulk and matters, if at all, on a sliver the benchmark cannot resolve.
...
### The benchmark gap: a determinacy audit {#audit}
