...
<figure>
  ...
  <figcaption><strong>Figure.</strong> The <code>inquire</code> skill: Peirce's three modes run as a procedure (<code>pmem</code>) that writes the hypothesis graph (<code>smem</code>). Abduction proposes hypotheses with kill conditions, deduction derives the exact trial each must survive, induction runs it against the world and fires a deterministic kill or witness with no model arbitrating. Each node is typed by the mode that set it and the credence cap travels with the type; the loop re-enters with the updated graph until a node is witnessed. The name honors Peirce's <em>inquiry</em>, not investigation. <code>implement</code> and <code>attest</code>, the stages that read the surviving hypotheses and verify the patch, follow below.</figcaption>
</figure>
