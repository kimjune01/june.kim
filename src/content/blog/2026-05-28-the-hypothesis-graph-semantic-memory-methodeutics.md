...
### What a coding benchmark can't see {#bench-blind}

A SWE-bench-shaped coding benchmark hands the solver three things: the issue text, the repository at the buggy commit, and a hidden test the fix must turn from red to green. That third gift removes the whole difficulty of the regime this paper is about. The failing test localizes the bug, pins the behavior the fix must produce, and supplies a cheap oracle to iterate against, so the one expensive step, deciding what is actually wrong and what the correct general fix is, comes handed over inside the problem. A method whose job is that diagnosis has nothing left to do.

The numbers show it. Run on SWE-bench Pro, the assembly resolves 95.3% of the public split under the official grader, and that number measures oracle availability, not the method. The failing tests are visible, the gate iterates against them, and any competent coding agent handed that signal reaches the mid-nineties. A preregistered oracle bracket prices the gift, and every diagnostic channel we could cut came back near zero on top of it:
...
The shape is not special to one benchmark. Every SWE-bench-shaped set, SWE-bench Verified, SWE-bench Pro, SWE-rebench, ships a hidden failing test with each task, so each hands the solver the same diagnosis the method exists to produce. A determinacy audit makes the dependence precise: across the family it is the hidden test, not the issue text, that settles which fix counts. The null is the bench unable to see a diagnostic method at all, the predicted signature of a mechanism that fires only where a benchmark cannot reach (full per-channel receipts in the [bench repository](https://github.com/kimjune01/swebench-pro)).

Where a benchmark cannot reach is an undiagnosed bug whose correct fix is a predicate, not a case, with no cheap oracle and a genuinely hidden cause. Far from a corner case, that regime is most of software, every issue tracker a backlog of undiagnosed problems waiting on the one expensive step a benchmark never exercises. The mechanism experiment ([github.com/kimjune01/hygraph-mechanism](https://github.com/kimjune01/hygraph-mechanism)) reconstructs that regime under control and runs the contrast a benchmark cannot, on a single bug dissected end to end rather than a population rate. This is also why the case is one chosen bug and not a sample: the property that makes the mechanism visible is the property a benchmark grades away, so the case has to be selected for it. Picking the bug is the experiment, not a thumb on the scale.
