...
*Recall?* The bug postdates every solving model's cutoff, and the recall probe comes back empty: asked in isolation how #2219 was fixed, the model does not recover the patch (§(verus-bench)).

The whole ablation reduces to one causal diagram. Across its arms, everything stays fixed but the verdict source. The gate's machinery is self-built (a second model rebuilt it from a leak-free prompt), but its calibration reference, the correct verdict for each probe, is the one piece the model cannot abduct for itself. That missing comparator is what an agent on its own could not reach, and the diagram below traces the route. Implementation gates the outcome too: codex had the comparator and still walled.
...
The correct verdicts come from the maintainer's own merged fix, so reaching them is human-level, on a bug whose fix postdates every model's cutoff. Three of four match them on every probe. codex clears the bug arm but walls on the divergence case, an implementation limit the correct verdicts do not remove. One *n*=1 cell per workflow, a mechanism existence-demonstration. It ranks no capabilities.
