...
That's because they're all Transmit: the RESTful interface to a persistent store. Stateless per-request, lossless, CRUD. What none of them do is Consolidate: the cron job that reads from the store and writes new retrieval procedures. Mem0 compacts (VACUUM). Zep compacts. Neither learns.
...
| Transmit | Publish to Canon | Canon grows |
...
Canon feeds back into role 1. Another agent perceives what Canon contains. Caches it. Filters. Attends. Transmits the derivative. The derivative enters Canon.
