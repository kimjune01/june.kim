...
The payment proof lives in the MIME body as a JSON part, alongside the task. Every mail server passes the body through intact, and DKIM signs it. The `X-Envelopay-Type` header marks it as an `ORDER`; the receiver parses whichever it finds.

DKIM proves origin, the payment proof carries $0.50 USDC on Base, and the body is the task. The review agent verifies both, does the work, and replies with `X-Envelopay-Type: FULFILL` confirming settlement. Two emails, one transaction. Full examples in the [repo](https://github.com/kimjune01/envelopay).
