...
An agent's reasoning can be given a typed, persistent, replayable substrate at the harness layer. The substrate is the **hypothesis graph** (hereafter *hygraph*): a semantic memory (`smem`, in the Soar sense) whose nodes are falsifiable hypotheses bound to recorded trials, whose edges are kill conditions that fire on evidence, and whose soundness invariant is replay: every node reconstructible from its trial by a reader who trusts nothing. The procedure that writes it is **methodeutics**, Peirce's methodology of inquiry, encoded as procedural memory (`pmem`): abduction proposes nodes, deduction derives their predictions, induction runs the tests, and a deterministic gate that no model arbitrates routes on the verdicts. We ran this assembly over the whole public set of SWE-bench Pro, twice, with the entire model pair swapped between runs, and committed a receipt for every instance: trajectory, graph, diff, gate trace, cost ledger. The resolve rates came back high, and the paper's contribution begins where the obvious reading of them ends. Controlled ablations return nulls on every channel that would credit the smem: prompt content is inert; deleting the entire diagnosis stage moves the rate by about a point; an oracle bracket attributes nearly the whole lift over bare models to the gate's access to the bench's visible tests. A determinacy audit of all 728 public tasks attributes the nulls to the instrument: roughly three quarters of the bench hands the solver the specification, so abduction has nothing to abduce; a proven 11.4–15.0% grades recovery of an unstated authorial choice; and the cheap, trustworthy oracle lets blind iteration substitute for diagnosis. SWE-bench Pro structurally cannot measure the mechanism it was chosen to measure, and its nulls are the predicted signature of that mismatch, not a refutation. In the regime that can measure it (real filed issues, no handed spec, no cheap oracle), eight of nine controlled pilots null against a strong minimal baseline, and the ninth is an audited divergence: the graph arm produced the general, sound fix, the minimal arm a suite-green over-narrow one, the difference settled by a one-command receipt and every load-bearing node replayed on a clean build. What survives is the paper's actual claim: the hypothesis graph as the unit of accountable agent reasoning, a trace a hostile reader replays instead of a verdict they are asked to trust. Underneath sits the norm this paper is written to declare: merit attaches to the work, not the doer, and the hygraph is vocabulary that makes the separation enforceable.
...
We tested the assembly the way the field tests everything: on the standard ruler. SWE-bench Pro is the contamination-resistant benchmark OpenAI now recommends in place of the saturated Verified. We ran the whole eligible public set under one frozen harness and the official grader, twice, with the entire model pair swapped between runs, and committed a receipt for every instance: trajectory, hypothesis graph, captured diff, gate trace, cost ledger (§(setup)). The numbers came back high: 95.3% with a frontier pair, 93.1% with open-weight models at ~12.6× lower cost (§(results)).

This is the second version of this paper, and it reports a reversal. The first version read those numbers as evidence that the method earns the lift, and the introduction asked why the field was not measuring harnesses. The attribution program that followed answered a different question than the one we posed. An oracle bracket, a prompt ablation, a whole-stage deletion, and a determinacy audit of all 728 tasks converged on one verdict: on this bench, the lift belongs to the gate's access to a cheap and trustworthy oracle, plus generic agent engineering; the typed inquiry the harness exists to encode is structurally unmeasurable here (§(attribution), §(audit)). The bench hands over the specification on roughly three quarters of its tasks, so there is nothing to diagnose, and grades an unstated authorial choice on a proven tenth, which no diagnosis can recover. The nulls are not a failure of the experiment. They are the predicted signature of a diagnosis mechanism pointed at a bench with no diagnosis in it.

I had believed that performance on benchmarks was a requirement for legibility. That may still be true for reach. But by this paper's own standards, a resolve rate turned out to be the least insightful artifact the work produced: a number that compresses 728 inquiries into one digit pair cannot surprise anyone into a hypothesis. A live example that can be abducted on can, and the work eventually produced one (§(right-regime)): an open, maintainer-stuck compiler bug where the graph arm's fix generalizes and the minimal arm's does not, the difference invisible to every test the project ships and settled by a receipt any reader can rerun. The bench bought attention. The example carries the insight.

So the paper now claims, in order of confidence:
...
4. **A mechanism finding, existence-grade**: in the right regime the hygraph's value is real, small in frequency, and lives precisely where oracles cannot see: fix generality, soundness, and a reasoning trace auditable at arbitrary depth (§(right-regime)).
...
This work is unincentivized public research. Its only standing is the auditable trail, so the losing path is documented as the richest artifact rather than buried, and every claim above ties to a committed receipt.
...
<figure>
  ...
  <figcaption><strong>Figure.</strong> Wall-clock per instance, all 728 eligible Pro instances. One peak at 10–15 min (median ~13 min; 84% finish inside 5–20 min), then a thin tail of heavy repos and craft-hangs on large suites.</figcaption>
</figure>

### Cost profile {#cost-profile}

The per-instance bill splits cleanly by role. The generator (Sonnet 4.5, carrying `inquire` and `implement`) accounts for $4.73; the blind challenger (GPT-5.5, the cross-family critique inside `implement`) accounts for $0.42. Verification independence, the harness's refusal to let the generator grade its own work, therefore costs about 8% of the per-instance total on the frontier pair. Within the generator, cache-read dominates: across the run, 5.3B cache-read tokens ($1,583) against 67M output ($1,011), because the typed stages and the outer loop re-read the hygraph and the trajectory rather than regenerate them. The open-weight pair holds the same shape at 12.6× lower cost: the structure ports, the weights get cheaper. Run totals and the two replication modes are in §(cost-envelope); the full per-leg derivation is in `COST_BASIS.md`.
...
Every choice below follows from one goal: making the run attributable. The harness is held fixed while everything contestable about it is exposed to audit. One frozen harness faces every run; grading stays with the official harnesses, so no scoring degrees of freedom are the operator's to spend; each instance leaves a provenance trail recomputable by someone who trusts none of these claims. That discipline is what later allowed the headline to be taken apart honestly: the same receipts that certify the number are the ones that localize what earned it.

### Prework and freeze {#prework}

The design follows from a diagnosis run before any number was frozen. On Verified the harness resolved 426 of 438 (97.3%), twelve not-won, and the open question was whether the twelve were capability misses or variance. Retrying those instances toward 100 would be training on the test set, so LLM subagents abduced a cause for each from its committed artifacts, then tested each cause by rerunning the instance under identical conditions. The reruns refuted the first hypothesis, that the gap was diagnostic: eleven of the twelve were variance (transient timeouts and capture losses that clear on a clean rerun), leaving one genuine capability floor. The design mechanizes the reliability conditions with a deterministic gate and capture-at-timeout.

Because the diagnosis came before the freeze, no part of the design was tuned to the Pro scores reported here. The eligibility rule, the one-shot held-out grading, and the requirement that every restart names its failure class before it runs are pre-registered in `PREREGISTRATION.md` and recapped under §(provenance).

### Datasets and eligibility {#datasets}

Two benches run under one frozen harness. **SWE-bench Verified** is a curated subset, saturated (top public submissions resolve >85% as of mid-2026) and training-cutoff-contaminated for current frontier models; OpenAI, which created Verified, [stopped reporting the score](https://openai.com/index/why-we-no-longer-evaluate-swe-bench-verified/) in February 2026 and recommends Pro instead. We use Verified as a baseline that the loop ports cleanly, with companion repo `swebench-verified` and a frozen artifact under Zenodo DOI. **SWE-bench Pro** is the live contamination-resistant tier with a public/held-out split across different repos, and it is the primary validation surface. Both runs use the official harness; no bespoke graders. Defect lists are documented per bench (`KNOWN_BAD.md`) and cover bench defects only, never our failures; the eligible set is the full set minus documented defects.
...
*Stage executors.* No stage calls the model bare. Each one invokes the provider's shipped agentic CLI: the Sonnet generator runs inside Claude Code, the GPT-5.5 challenger inside codex, and in the open-weight run the generator runs inside Cursor (Composer is itself an agent). Each of these is a ReAct loop of the vendor's own, with its own tools, file access, and turn budget. The harness is therefore a typed meta-loop over off-the-shelf vendor agents: `inquire`, `implement`, and `attest` each delegate to one, and the harness owns only the typed stage contracts, the hygraph, and the deterministic gate between them. The bare-baseline comparison consequently swaps the inner agent and adds the typed meta-structure at once; it was never a clean isolation of the typing, a confound §(attribution) resolves from the other direction.

*Oracle boundary.* On the public split, the bench's `FAIL_TO_PASS` tests are visible. The frozen harness's in-container gate executes them as its stop signal; their *names* function as budget control, and their bodies are never placed in the model's prompt. The standardized leaderboard scaffold has no such access: Pro hands the agent the problem statement, requirements, and interface, and holds the tests out entirely. That difference is the dominant attribution channel (§(oracle-bracket)), and Pro's held-out split removes even the names, so nothing in this paper's public-split numbers transfers to the private set.

Two configuration details worth disclosing, since reasoning budget is itself a labeled axis on the public boards. The Sonnet 4.5 generator ran with extended thinking enabled at Claude Code's default level. The `implement` challenger (codex GPT-5.5) ran with `reasoning_effort` unset, per the `turn_context` of every committed rollout. That the challenger arbitrates with its reasoning off is a point against a model-deliberation reading of the gate.

*Prior-instance information boundary.* For each scored bench instance, the loop reads only artifacts derived from that instance's failing-test reproduction and its own in-cycle writes; no other instance's hypothesis graphs, trajectories, or solutions enter the model context.
...
### Scoring discipline and provenance {#provenance}

The harness is under test; infrastructure and model-provider reliability are not, so the retry rule re-runs only the failures the harness does not own. A failure the harness produces stands as a loss; a platform fault re-runs only when independent evidence corroborates it, and an uncorroborated fault reclassifies to a loss, closing the one move that could launder a capability loss into a free retry. The held-out set is graded once on the frozen artifact and never read as a stopping signal. The artifact is frozen by an annotated git tag, and every restart names the failure class that justified it. Each instance commits its full trail: trajectories, hypothesis graph, captured diff, gate trace, grader output, cost ledger. The complete state machine, fault classes, corroboration rules, and a doubt-by-doubt verification guide live in the repository (`PREREGISTRATION.md` §4, `FOR_SKEPTICS.md`, `OBJECTIONS.md`).

### Cost and replication envelope {#cost-envelope}

Two replication modes are validated in this work, and this run uses both in parallel. *Subscription mode*: a single frontier-vendor consumer subscription (~$200/month) covers the run at a multi-week pace, gated by per-window quota; marginal cost at the boundary is zero. *API mode*: pay-as-you-go at the published rate, ~$5.14 per instance for the frontier pair (~$3.7k economic across the Pro run); observed cash ran far lower under subscription subsidy (~$870 marginal, derivation in `COST_BASIS.md`). Compute outside the model is minimal: 4× small EC2 instances, no GPU, no curated training data. A consumer subscription, a low-four-figure API budget, or about $300 for the open-weight pair puts replication within reach of graduate students and independent researchers.

### Open-weight robustness run {#open-weight-run}

The same frozen harness was rerun on Pro with an open-weight generator: Cursor Composer 2.5, which [Cursor states](https://cursor.com/blog/composer-2-5) is a fine-tune of Moonshot's open-weight Kimi K2.5, paired with Gemini Flash 3.5 as the challenger. The generation lift runs on weights anyone can download and self-host; the cross-family adversarial property (§(blind-blind)) is preserved (Cursor/Moonshot × Google).
...
<tr><td>Median wall-clock</td><td>12.8 min</td><td>8.4 min</td></tr>
<tr><td>Cost / instance</td><td>$5.14</td><td>$0.41</td></tr>
</table>

The raw open-weight rate is partly recall-inflated: a gold-overlap audit ([`gold_divergence.py`](https://github.com/kimjune01/swebench-pro/blob/main/driver/gold_divergence.py)) finds ~23% of the open-weight wins reproduce gold's changed lines at high overlap, against ~2% for the frontier pair. The asymmetry is the tell: a weaker model reproducing the human gold patch more often than stronger ones points to recall, not capability. Discount the entire near-gold tail and the harness still carries the cheap model to roughly three-quarters genuine resolve (~71 to 76%). The run is read as cost and portability, never capability, and it carries the same provenance contract as the headline run under the same bundle.

## Results: the number, bounded {#results}

> Both surfaces are closed. Verified: 426 / 438 eligible (97.3%), frozen, Zenodo-DOI'd, public. Pro: 694 / 728 eligible = 95.3%, 0 incomplete, whole eligible set graded under the official grader at frozen tag `prereg-pro-v1`; open-weight pair 678 / 728 = 93.1% at tag `prereg-pro-v1-cheap`.
...
- **Most wins are first-pass.** Of the 694 wins, 648 carry a complete captured trajectory; among those, the first `inquire` → `implement` → `attest` pass carries ~93% (602) and outer-loop re-entry recovered the remaining ~7% (46). Within a single first pass, `implement` checks its patch against the gate a median of once and iterates (2–10 runs) on 40% of wins, a within-pass refinement loop that §(attribution) shows is most of the story.
- Run span ~3.5 days through three provider-credential stalls and a mid-run billing switch, with 0 instances lost.
- Per-repo (11 public repos): ten of eleven resolve at 92.3% or above; NodeBB is the lone outlier at 74.4%.
- **No development-overfit signal.** The harness was iterated on Verified (all Python) before this run; the development language resolves *lower*: Python 94.7% versus non-Python (Go/TS/JS, never touched in development) 95.7%.
- **All 34 losses have non-empty captured patches**: every loss is the loop producing a fix the official tests rejected, not failing to produce one.
- **The WINs reproduce on re-grade**: 6/6 cross-language frontier WINs and a 60/60 stratified open-weight sample, 0 flips, applying the captured diff to the unmodified official grader on a clean container (`driver/regrade_wins.py`). At ~$3 per WIN, a full independent re-grade is within anyone's reach.

*OSS deployment trace.* **81 PRs merged across 73 cold repositories** (codebases the operator does not own, 0 self-owned, median merged diff 49 lines) under adversarial maintainer grading; a **50.6% merge rate** (81 of 160 decided PRs under the `sweep` campaign tag). A close-reason audit found only ~8 of the 79 closures were rejections on the merits. No human-authored code in any merged PR, no prompt-level steering, no human curation of which issues to work on: the harness samples issues at random from ≥200-star repos across ~9 languages and runs the full loop autonomously. The frozen ledger ships in the artifact (`pr-receipts.jsonl`) and stays GraphQL-verifiable against live GitHub; ~385 hypothesis graphs are publicly committed at [`kimjune01/sweep/repo-hypotheses/`](https://github.com/kimjune01/sweep). The full deployment story is in [Speedrunning Open Source](/speedrunning-open-source).

Three receipts, three independent attestors:
...
<tr><td>Economic cost</td><td>~$5.14/instance frontier, ~$0.41 open-weight; ledger published</td><td>anyone, recomputing at public API rates</td></tr>
</table>

We do not attest; the receipts do. No method documented in our comparative literature search (§(search)) has *demonstrated*, with equivalent receipts, a higher SWE-bench Pro official-grader resolve rate at a lower audited per-instance dollar cost under one frozen harness. That claim is about the artifact and its auditability, and it survives the attribution below. What does not survive is any reading of the rate as evidence for the method.

<figure>
  <svg viewBox="-40 0 880 360" role="img" aria-label="Sankey flow of 728 eligible SWE-bench Pro instances: 694 resolve and 34 do not; of the resolved, 602 are solved on the first pass, 46 are recovered by the outer loop, and 46 predate trajectory capture." style="max-width:760px; width:100%; height:auto; margin:1em auto; display:block; font-family:system-ui,-apple-system,sans-serif;">
  ...
  <figcaption><strong>Figure.</strong> Where the 728 eligible Pro instances land. The first pass carries <strong>602</strong> wins; the deterministic outer loop converts another <strong>46</strong> first-pass misses, and 46 further wins predate trajectory capture.</figcaption>
</figure>
...
The first version of this paper asked what earns the gap and answered honestly that it could not fully decompose it. The completed attribution program can. Four controlled cuts, one channel at a time, each preregistered or receipt-committed:
...
The largest channel is the one the original design held constant and therefore could not see. On a preregistered 50-instance random sample, an implement-only loop with no oracle access floors at **50%**; restoring the bench's tests for the gate to iterate against raises the ceiling to **96%** ([`PREREGISTRATION-baseline-replication.md`](https://github.com/kimjune01/swebench-pro/blob/main/docs/PREREGISTRATION-baseline-replication.md), [the bracket](https://github.com/kimjune01/swebench-pro/blob/main/docs/HYPOTHESIS_GRAPH-pro-harness.md)). Forty-six points sit between those arms, bought by oracle access, not by the same models reasoning harder. The floor sits near the bare-model leaderboard scores (43.6–64.3%); the ceiling sits at the headline. The held-out tests account for essentially the whole gap, a conclusion the companion posts reached first ([Precisely Wrong](/type-iii-error), [How Not to Run SWE-bench Pro](/how-not-to-run-swebench-pro)) and the remaining ablations refine: given the oracle, what does the rest of the structure add on this bench? Channel by channel: almost nothing.

### The prompt is inert {#prompt-ablation}

Holding the harness byte-identical, we varied only the diagnosis prompt across three arms: **M**, the methodeutic prompt (rival hypotheses, discriminating tests, kill-the-fix, the abduction–deduction–induction taxonomy); **G**, a steelman of generic engineering rigor carrying none of the methodeutic vocabulary; and **T**, a task-only floor. To guard against an experimenter-weakened control, G and T were authored by a third-party model (GPT-5.4) handed M as the parity target and told to win; the methodeutic vocabulary is present in M and absent, by grep, from G and T.

We scored each arm's diagnosis directly against the gold patch by base-side line-region overlap, recon-only, before any implementation. The three arms are indistinguishable: M − G is −0.012 (95% CI [−0.069, +0.045], n=38) and G − T is +0.035 ([−0.023, +0.092], n=36).[^promptnull] The methodeutic vocabulary reads no better than generic rigor, and generic rigor no better than the bare task. We are not claiming our prose reasons better, and the result is scoped to diagnosis recall on this bench; but it cleanly excludes prompt wording as the channel.

[^promptnull]: A single-instance pilot had hinted that the structured prompt (G) beat the minimal one (T); powered to n=36 the gap vanished. The full trail, including two infrastructure contaminations caught and re-run, is in the repository worklog.

### Directed perturbation, and the gate's second role {#perturbation}

One channel does bite, conditionally. A static-deprived arm (the aimed diagnostic probe cut from `inquire`, blind search left untouched) against the frozen perturbing baseline, stratified before treatment by whether the issue's cause is statically determined from its text: on underdetermined causes, removing directed perturbation costs a small positive amount (Δ = +0.105, 95% CI [+0.01, +0.20], P(Δ>0) = 0.98, n=76); on determined causes it is near zero (Δ = +0.03, n=34). The exact McNemar test lands just over the conventional cutoff (p = 0.057): threshold-level, not decisive, with the win-rate instrument near the benchmark's resolution limit.[^perturbretract] The stronger support is the conjunction with eleven gate-confirmed existence cases and the predicted specificity by cause class, a pattern harder to fake than uniform magnitude.

[^perturbretract]: An earlier headline (Δ = +0.278, P = 0.996) was retracted: it was inflated by authentication-death contamination, where pipeline failures before the gate were miscounted as deprived-arm losses. The full retraction and the re-run are in the repository worklog.

Why does removing directed perturbation cost so little? Because something substitutes for it: the deterministic gate, in its second role. Directed perturbation's only advantage is query efficiency, one aimed experiment instead of many blind ones to tell rival hypotheses apart. Blind try-and-check inside `implement` gathers the same information by trial, but only if each trial is cheap and its result trustworthy, which is exactly what the gate supplies. There is a methodological twist worth stating plainly: the gate was held constant across both arms as a controlled-for covariate, so the very thing we fixed to keep the ablation clean is the channel that routes around the cut. And it sharpens into a falsifiable differential prediction: degrade the gate (flaky tests, no oracle, expensive evaluation) while perturbation stays absent, and the deprived arm should worsen more than the perturbing arm. The regime that flips the result is not easy-versus-hard cases; it is gate-present versus gate-absent. §(right-regime) runs that regime.

### Pricing the diagnosis stage {#craft-only}

If a cheap gate lets blind iteration substitute for the aimed probe *inside* diagnosis, does it substitute for the whole diagnosis stage? The **craft-only** arm deletes `recon` entirely: no hypothesis graph, no typed inquiry, no diagnosis; `implement` receives only the raw problem statement and the failing tests. Everything else is the frozen harness.

Paired against the frozen baseline on a stratified sample (18 determined, 18 underdetermined causes), removing the stage barely moved anything. Determined: Δ = +0.000 (n=18). Underdetermined: Δ = +0.062 (n=16), the entire effect one instance. The only clear divergences were two existence cases splitting in opposite directions: on `element-web-66d0b318`, craft-only **won** a case the perturbation-deprived arm had lost (no diagnosis beat a wrong one); on `qutebrowser-e34dfc68`, craft-only **lost** after 54 minutes a case the full pipeline won (no diagnosis was worse than a good one). Reweighting the stratum rates to the population puts craft-only near **94%** against the full harness's 95.3%, a gap of roughly one point that could be anywhere from zero to two.

On this bench, the smem is redundant on the bulk and matters, if at all, on a sliver the benchmark cannot resolve.

### The attribution verdict {#attribution-verdict}

Sum the channels. The oracle the gate iterates against buys ~46 points. The prompt vocabulary buys nothing. The aimed probe buys a threshold-level sliver on one stratum. The whole diagnosis stage prices at about one point. The model pair, the lever the leaderboards exist to rank, moves the raw rate two points (more on genuinely novel problems). On SWE-bench Pro, the typed inquiry this harness exists to encode is not load-bearing, and the original reading of the headline, the lift as the method's, was wrong. The honest question stopped being *how is the harness this good* and became *why doesn't the bench register the mechanism at all*. That question has a mechanical answer.

## The instrument: a determinacy audit of SWE-bench Pro {#audit}

A benchmark score is only as meaningful as the determinacy of its tasks: if the materials a solver receives do not pin the behavior the hidden test grades, then passing is not evidence of solving the stated problem but of recovering an unstated choice by other means. We audited all **728 public Pro tasks** for exactly this, as a companion artifact with its own receipts ([github.com/kimjune01/swebench-pro-audit](https://github.com/kimjune01/swebench-pro-audit)): every graded behavior is checked against the prose (problem statement, requirements, interface) and against the live codebase at the base commit, with grep-certified witnesses, blind two-family adversarial verification for the judgment-bearing tier, and a refuted-negative-control set. The audit is self-authored, and citing it costs nothing epistemically: each row is a pointer to an ambiguity a hostile reader re-derives by grep from the committed gold, test, and prose, nothing taken on our word.

<table style="max-width:740px; margin:1em auto; font-size:14px;">
<colgroup><col><col style="width:6em"><col style="width:6em"><col></colgroup>
<thead><tr><th style="background:#f0f0f0">Label</th><th style="background:#f0f0f0; text-align:right">count</th><th style="background:#f0f0f0; text-align:right">of 728</th><th style="background:#f0f0f0">meaning</th></tr></thead>
<tr><td>ENTAILED</td><td style="text-align:right">478</td><td style="text-align:right">66%</td><td>every graded behavior has a covering requirement: the spec is handed over</td></tr>
<tr><td>DETERMINED-codebase</td><td style="text-align:right">78</td><td style="text-align:right">11%</td><td>prose silent, but the codebase determines it one way and gold matches</td></tr>
<tr><td><strong>Proven underdetermined (mechanical spine)</strong></td><td style="text-align:right"><strong>83</strong></td><td style="text-align:right"><strong>11.4%</strong></td><td>graded constant absent from everything a solver reads, or the codebase determines a value the test contradicts; re-derivable by grep</td></tr>
<tr><td><strong>+ two-expert splits (adversarially verified)</strong></td><td style="text-align:right"><strong>26</strong></td><td style="text-align:right"><strong>→ 15.0%</strong></td><td>prose or source licenses ≥2 faithful readings; the hidden test splits them; constructed by one model family, survived refutation by another</td></tr>
<tr><td>Screen-flagged, rater-pending (not counted)</td><td style="text-align:right">63</td><td style="text-align:right">9%</td><td>candidates excluded from every claim</td></tr>
<tr><td>KNOWN_BAD / KNOWN_MISMATCH</td><td style="text-align:right">3 / ≥1</td><td style="text-align:right"></td><td>gold fails the grader / prose describes one feature, gold and test grade another</td></tr>
</table>

The mismatch class is the audit in miniature. `flipt-io_358e13bf`'s problem statement asks for controlled deletion of references in a snapshot cache; the gold patch and hidden test assert a CSRF configuration default. No reading of the prose produces the behavior the test checks. An extreme case, but it makes the general point: passing the test and solving the stated problem can come apart, and the gap is measurable.
...
The conclusion is structural, and it indicts no one's honesty: a spec-conformance bench with a cheap visible oracle and a measurable lottery fraction cannot measure a diagnosis mechanism, whoever runs it. The nulls of §(attribution) are what a true mechanism looks like when measured on an instrument that cannot see it. The practical implication for the field is the audit's headline: a raw Pro percentage conflates solving the stated problem with recovering the author's unstated choice, and should be reported against a determinacy-aware denominator.

The instrument generalizes. The audit method ships as a portable tool, [**determinacy**](https://github.com/kimjune01/determinacy): point it at any SWE-bench-shaped dataset (a TOML field-map; SWE-rebench is already run, Verified and Pro configs included) and it labels every graded behavior, certifying the underdetermined ones with receipts a skeptic re-runs by grep. Its name for the conflation is the **specification lottery**: a binary bench scores the agent for reproducing one maintainer's choices, so when the issue underdetermines the fix, a correct patch scores zero and a leaderboard cannot tell capability shortfall from spec shortfall. The tool claims only the mechanical floor, because the vibe-check alternative (ask an LLM "is this ambiguous?") over-flags ordinary engineering latitude and produces big, useless numbers; separating what is provable from what is a rater's opinion is the entire job.

## The right instrument {#right-regime}

The gate-compensation mechanism predicts where the smem should matter: where the oracle is expensive or absent and the cause is genuinely hidden. That regime exists and we had already been running in it. The deployment lineage (§(results)) works real filed issues: no handed spec (the symptom is whatever a bug reporter wrote), no cheap oracle at solve time (the grader is a human maintainer), real diagnosis required (the cause is hidden in a cold codebase). The mechanism experiment ([github.com/kimjune01/hygraph-mechanism](https://github.com/kimjune01/hygraph-mechanism)) reconstructs that regime under control and runs the ablation Pro made meaningless: same model, same loop, the methodology the only variable, an existence-proof burden rather than a population rate.
...
Nine pilots in, the score is honest and uncomfortable in both directions:
...
The calibration is strict: n=1, existential, decisive on this instance and on no rate. What it establishes is the shape of the mechanism's value, and the shape is the finding. The graph's advantage did not appear as pass-versus-fail on the original bug. It appeared as correctness invisible to the oracle, settled only by a receipt, carried by a trace auditable at arbitrary depth. That is also the experiment's own lesson applied to itself: eight loops ran as inductions with no per-loop deductive rung, so every null retrofitted into a new abduction instead of testing one; the program now preregisters one sentence per loop (testing X, predict Y, refuted by Z) before any arm runs (`METHODOLOGY-preregistration.md`).

## Discussion {#discussion}

### What survives of the bench result

The artifact stands, bounded. A frozen, fully auditable harness resolved 95.3% of public Pro under the official grader at ~$5.14 per instance, replicable on a consumer subscription, and the same structure carried an open-weight pair to 93.1% at $0.41. The practitioner guidance is real: for spec-given work with a trustworthy test oracle, a cheap model inside a gate-disciplined loop is the cost-effective configuration, and the frontier pair earns its premium only on the novel tail. The receipts discipline (preregistration, freeze, fault classes with invariants, official-grader-only verdicts, per-instance provenance) is what let this paper reverse its own headline without a credibility crisis, which is the strongest argument for the discipline we know how to make. Publishing nulls is part of it, and these nulls publish *attributed*: each arrives with the mechanism that produced it (the gate compensating, the spec handed over, the lottery fraction). An unexplained null says stop; an attributed null says where to point the next instrument.
...
- **Replication budget.** ~$5.14/instance frontier (~$3.7k across Pro), ~$0.41 open-weight (~$300), a single re-grade ~$3/WIN; or a consumer subscription plus patience (§(cost-envelope)).
...
The artifact claim (§(tables)), *no method documented has demonstrated a higher SWE-bench Pro resolve rate at a lower audited per-instance cost with equivalent receipts*, requires a comparative search. The bar for *equivalent receipts*: published per-instance trajectories, captured diffs, gate or evaluator traces, cost ledger, and reproducible run conditions.
...
- **Meehl 1967**; **Feynman 1974** ("Cargo Cult Science"): the difference between rigor-shaped activity and actual rigor, which §(attribution) relearned at first hand.
- **Kimball 1957; Tukey**: the Type III error, the exact answer to the wrong question; the category this paper's first version occupied and its companion post [Precisely Wrong](/type-iii-error) examines from the inside.
