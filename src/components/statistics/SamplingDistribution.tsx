...
export default function SamplingDistribution() {
  ...
  return <section className="stats-lab" id="means-lab" aria-label="Build a sampling distribution">
    ...
    <label className="stats-control">Observations per sample<select aria-label="Observations per sample" value={size} onChange={e => { setSize(Number(e.target.value)); setMeans([]); }}>{[1, 2, 5, 10, 30, 100].map(n => <option key={n} value={n}>{n} observations</option>)}</select></label></div>
