\begin{abstract}
Large language model (LLM) assistants create an advertising surface
where conversations express intent beyond keywords. We develop an
auction that combines targeting preferences and willingness to pay in a
single report. Each conversation embeds as a point \texttt{x} in a
continuous space of hundreds of dimensions, and each advertiser declares
a center \texttt{c} (their customer). The conceptual distance between
them measures how closely they match. A reach \(\sigma\) (how widely
they serve) and a bid \texttt{b} (what a conversion is worth) compare
that distance against a willingness to pay. The slot beside the reply
goes to the advertiser with the highest
\(\log(b) - \|x-c\|^{2}/\sigma ^{2}\). The mechanism involves no model
training or response modification. We prove in Lean 4, with zero
\texttt{sorry}, that the rule's argmax allocation with Clarke payments
forms a VCG mechanism. Truthful reporting is weakly dominant, the
allocation maximizes welfare at every query point, and its territories
form a power diagram with keyword auctions as the degenerate case. This
mechanism allows an auction to sell regions of embedding space.
\end{abstract}
...
We connect a geometric bidding language to VCG and verify that
connection in Lean. The formalization covers truthful reporting of
center, reach, and bid. It identifies the allocation as a power diagram,
using a lift when reaches differ, and recovers the single-item Vickrey
auction at a shared center. These guarantees can be checked by
\hyperref[formalization]{running a build command}. The bridge lemma,
\texttt{score\_eq\_log\_reportedVal}, shows that the scoring rule is the
logarithm of the value a report implies, without assuming truthful
reporting. This identity connects the allocation to the classical
incentive argument of \href{https://doi.org/10.2307/2977633}{Vickrey
(1961)}, \href{https://doi.org/10.1007/BF01726210}{Clarke (1971)}, and
\href{https://doi.org/10.2307/1914085}{Groves (1973)}.
...
Advertisers express their positioning in natural language and refine it
by inspecting example queries, as described in
\href{https://june.kim/buying-space-not-keywords}{Buying Space, Not
Keywords} and
\href{https://june.kim/marketing-speak-is-the-protocol}{Marketing-Speak
Is the Protocol}. The mechanism represents that positioning with a
center, a scalar reach, and a bid. We leave controls for specifying more
detailed preference geometry outside the scope of this work. Separately,
we assume true valuations belong to the isotropic Gaussian family; the
theorems hold within that model. The platform scores reports by
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={The upper envelope of three reported-value surfaces over a two-dimensional embedding space. Height shows reported value and color identifies the winner. Tall, narrow A and intermediate C win near their centers; broad, lower-bid B wins the surrounding queries. Curved color boundaries mark ties between the highest reported values.}]{vcg-fig1.pdf}}
\caption{The upper envelope of three reported-value surfaces over a two-dimensional embedding space. Height shows reported value and color identifies the winner. Tall, narrow A and intermediate C win near their centers; broad, lower-bid B wins the surrounding queries. Curved color boundaries mark ties between the highest reported values.}
\end{figure}
...
\textbf{Theorem (Dominant-strategy incentive compatibility,
\texttt{vcg\_dsic}).} Fix a query and the reports of all other
advertisers. Under the stated allocation and payment rules, an
advertiser whose true valuation belongs to the Gaussian family obtains
at least as much quasilinear utility by reporting its true center,
reach, and value as by submitting any alternative valid report.

The Clarke payment depends only on others' reported values, so a
player's report moves their allocation and never their price schedule
(\texttt{welfareOthersWithout\_invariant}); the four-case comparison
then closes the argument. A false center can make the supplement
advertiser win the running-shoe impression, but its utility still
contains the small true supplement-conversion value, while its payment
equals the value of the shoe advertiser it displaced. In the figure's
instance, the supplement advertiser S's books at the shoe query
\texttt{x}:
...
\hyperref[explorer]{The explorer} renders the cells live.

The lift also helps at serving time. Score comparisons are affine in the
lifted query \((x, \|x\|^{2})\), reducing winner selection to
maximum-inner-product search over static advertiser vectors. Approximate
indexes can retrieve candidates, but exact VCG clearing requires
certifying the winner and runner-up. A full O(N) scan provides the exact
fallback; the O(log N) point-location bounds available in fixed low
dimensions do not extend to embedding dimension.
...
The mechanism contains the single-item Vickrey auction as a special
case. Two results connect it to point-based keyword bidding: a report
concentrates at its center as reach shrinks, and allocation and payment
reduce exactly to Vickrey's rule when the query is every advertiser's
shared center.
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Shrinking \textbackslash sigma at a fixed center against a wide rival with equal bid. The value at the center is b for every \textbackslash sigma, while territory off-center collapses; in the limit the advertiser competes at one point only, on bid alone.}]{vcg-fig3.pdf}}
...
pays nothing (\texttt{vcgPayment\_eq\_zero\_of\_loser}). At a shared
center, the embedding auction is Vickrey's sealed-bid second-price
auction, allocation and payment both.
...
quantifiers). \texttt{gaussian\_vcg\_weakly\_dominates} brings welfare
optimality and dominant-strategy incentive compatibility together with
efficiency at truthful equilibrium. The equilibrium result follows by
evaluating the welfare guarantee at truthful play, which
\texttt{vcg\_strategyproof} certifies as a Nash equilibrium. Truthful
reporting maximizes each advertiser's utility against fixed rival
reports (\texttt{vcg\_dsic}). When all advertisers report truthfully,
the allocation maximizes total advertiser value.
...
Gaussian describes a region with one center, one reach, and one bid.

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Simulated conversion values against distance from the true center, drawn along two directions of embedding space. The isotropic Gaussian model assumes the same decay with distance in every direction. Natural-language positioning and example queries support report refinement; controls for more detailed preference geometry are outside the scope of this work.}]{vcg-fig5.pdf}}
\caption{Simulated conversion values against distance from the true
center, drawn along two directions of embedding space. The isotropic
Gaussian model assumes the same decay with distance in every direction.
Natural-language positioning and example queries support report
refinement; controls for more detailed preference geometry are outside
...
opposite limiting case. The public scoring rule determines the
allocation, and the formal incentive guarantee is independently
checkable.
...
\emph{Gaussian truth.} The isotropic Gaussian family gives the auction a
compact bidding language and tractable geometry. Choosing that language
trades expressiveness against clearing cost, as in combinatorial
auctions (\href{https://doi.org/10.1145/352871.352872}{Nisan, 2000}).
The incentive theorem assumes true valuations can be expressed in this
language. Anisotropic Gaussian preferences retain the logarithmic
bridge, but extending the formalization and geometric representation
...
charged rent for the translation. This report drops the keyword
translation while retaining a familiar auction at a shared center:
\hyperref[keywords-recovered]{Keywords Recovered} proves that allocation
and payment there reduce exactly to Vickrey's rule. The declaration also
