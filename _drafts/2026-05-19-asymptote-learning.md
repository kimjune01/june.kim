...
The hoist has a hard precondition the first two don't: the corpus has to already exist. PR-routing is the easy case because GitHub is millions of recorded decisions of exactly that shape; the supervisor rediscovers them rather than waiting to suffer them. A novel app has no such record. The decisions about what it should do haven't been made yet, by anyone, because the app is the thing being created. There is no dataset before the app exists. The corpus hoist accelerates categories where the world already decided at scale; greenfield work falls back to the slow local loop, accreting one operator decision at a time, exactly as expert systems were always built. The third hoist buys speed only where a public record already paid for it.
...
The asymptote, in a domain whose regularities reduce to code: **the encoded shell *is* the shape of the problem, and the LLM nucleus is exactly the dimension where the problem has no shape.** The mature stack is a problem-shaped shell at each recursion level with a thin LLM at every level filling the holes the shape leaves. The qualifier is essential. The shell only grows to the shape of the problem when the problem's regularities are the kind code can hold. [Neuro-symbolic surveys](https://www.ijcai.org/Proceedings/2020/688) argue for hybrid decomposition; continual-learning and tool-use papers argue that adaptation matters; none state this as the asymptotic property that follows once the encoding loop has clean interfaces and enough time. The contribution is to name expert-system construction as an online encoding loop with replay-gated transfer from public decision corpora, and to identify its asymptote.

## Generalization

The construction depends on PRs and GitHub only in the example. Any classification problem with these properties admits the same encoding chain: a clean structured interface in, a finite partition out, an invariant that every event reaches a terminal bucket, two attention channels for misclassification (ambiguous and false-known), and a substrate writeable enough to receive encoding updates at runtime. The load-bearing property is one more: regularities that reduce to deterministic checks. Given those, the encoding loop drives the system to a *policy fixed-point:* a function from inputs to buckets that the next encoding pass leaves unchanged. The monoidal contract from [Make No Mistakes](https://june.kim/make-no-mistakes) is the formal reason it converges: generalized online learning, where the policy has a fixed point because the substrate that implements it is a monoid under composition. The properties hold across engineered software systems (bug routing in a codebase, build dispatch, deploy gating, lint triage) where the same CLI-and-enum hoist applies. Ranking, scheduling, and dispatching share the structure wherever their distinctions are similarly code-shaped; where they aren't, the shell stays thin and the fixed point keeps a fat fuzzy core.

Which lets us land a definition with no mysticism left in it: **expertise is a complicated policy at its fixed point.** An expert is someone whose encoded shell has converged on the problem they work; the residue they hand to intuition is the part no further encoding could compress. The definition is structural, not a promise about size: where the problem reduces to code the residue is thin, and where it doesn't the fixed point can be almost all residue. Expertise isn't a substance the expert possesses; it's the trace of the procedure that produced them, terminated. Encoding expertise is that procedure, run on purpose.
...
This is why expert systems stayed popular for forty years after the AI-winter eulogies. They never went away; they got renamed. Every line of every app you use every day is a record of someone, or a group of someones, deciding what the machine should do in some specific scenario, accreted slowly through years of bug reports and feature requests and incident reviews. The Stripe dashboard is the encoded fixed point of "what should an operator be able to do with a payment." The deploy script, the CI gate, the linter config, the CRM workflow: each is a complicated policy at the local fixed point its maintainers reached, and each is made of code. The encoding loop in this trilogy is the same procedure, automated. The LLM's new role isn't to replace the expert. It's to build the expert system, one classifier at a time, at machine speed.

Step back far enough and the close lands, in the precise sense the title uses: the trilogy's expertise is the category expert systems already encode in software, systematic rule-application across scenarios that bottom out in code. PR-routing is the worked example, and its cousins are the rest of the category: bug routing in a codebase, build dispatch, deploy gating, lint and CI triage, dependency-bump adjudication. All are the same shape as the PR-routing the operator was doing by hand in the first essay, in a different uniform, and all share the one property that makes the shell grow: their distinctions reduce to a query against a system that already speaks code.

By contrast, the radiologist's eye, the violinist's hands, the novelist's chapter run on substrates the encoding loop does not reach. The harder cases sit between. Loan adjudication, contract redline, claims triage, content moderation look like classification, with structured-ish input, finite buckets, decision logs. But their regularities don't reduce to `if` statements, so the shell stays thin and the operator keeps carrying most of the judgment. The honest claim stops at the software boundary.

What lies past it is a conjecture, not a result. The same loop might *engineer the system itself*: build the formal substrate a fuzzy domain lacks, the schemas and checks and ground-truth tools that would let the shell grow, and so drag a domain across the boundary it didn't start on. That's untested. This trilogy demonstrates the loop only where code already holds the regularities. Whether it can manufacture that substrate where it's missing is the next experiment, not a claim banked here.
...
Which is as precise as a definition gets within the boundary. If we can encode the procedure and arrive at an artifact of the same form across the software category, we have enclosed the category that boundary defines: not all expertise, but the kind expert systems were always for. Expertise, in that sense, is no longer a fuzzy folk term; it is named by what builds it. The shell is the proof; it has been the proof all along, and how far the boundary can be pushed is now an empirical question, not a slogan.
