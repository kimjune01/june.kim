...
Given these constraints, any system that doesn't periodically store a complete state will drift past recovery. The checkpoint isn't an optimization. It's a necessity. The only free parameter is how often.

### The evidence

The table below shows the same five-part architecture appearing independently across fifteen domains. No domain copied from another. The technical systems (top rows) are the most precise; the institutional and cultural systems (lower rows) are less exact but structurally recognizable. The pattern holds across all of them.
...
| **Language** | Standardization event | Gradual drift | — | Mutual unintelligibility | Creolization / new standard |
| **Memory** | Episodic keyframe | Semantic delta | Sleep cycle | Interference / decay | Consolidation |
| **Motor learning** | Learned chunk | Variation / refinement | Practice session | Skill breakdown | Re-learn from scratch |
| **Scientific paradigm** | Paradigm (Kuhn) | Normal science | Research program | Anomaly accumulation | Revolution |
| **Software architecture** | Clean-sheet design | Feature additions | Major version | Technical debt collapse | Rewrite |

Fifteen domains. Same architecture. The top five are precise — the mapping is operational, not metaphorical. git's packfile delta compression is literally I-frame/P-frame encoding on a DAG. Database WAL checkpointing is literally GOP structure. The lower ten are structural analogies, but the same five components appear in each: a checkpoint, a delta, a cycle, a failure mode, and a recovery mechanism.

### What kills systems: the evidence

The keyframe rate is a survival parameter. Systems that get it wrong — too few checkpoints or too many — fail in predictable ways. The table below collects documented cases.
...
**1. Too few keyframes kills slowly.** Boeing drifted for 57 years. The Qing drifted for 268. Korea is drifting now. The failure mode is always the same: accumulated error becomes structural, and the system can no longer distinguish between what it is and what it was supposed to be. By the time someone calls for a reset, the cost of a keyframe has grown larger than the system can bear.

**2. Too many keyframes kills fast.** France cycled through 14 constitutions in 80 years and couldn't build legitimacy. Mao's Continuous Revolution destroyed institutions faster than they could function. The Khmer Rouge's Year Zero destroyed the substrate itself. The failure mode: each reset discards accumulated P-frames — institutional knowledge, precedent, cultural continuity — that took years to build. Reset too often and nothing accumulates.

**3. The survivors calibrate.** The Catholic Church resets about once per century. The British constitution never resets fully but patches continuously. The Vedas use redundant encoding as continuous error correction. Aboriginal songlines anchor to physical terrain that doesn't drift. Each found a keyframe rate matched to its error accumulation rate and its checkpoint cost. None found it by theory. All found it by selection: the ones that got it wrong aren't here to compare.

### Trauma compresses the keyframe

South Korea's population crisis illustrates a specific failure mode: a keyframe written under crisis that was never decompressed after the crisis ended.
...
Then the crisis ended and the I-frame didn't update. Family formation, communal obligation, generational continuity — everything the pre-war I-frame carried — was never re-encoded into the new reference. It existed only as P-frame inheritance, increasingly distant from the active checkpoint. When accumulated economic and social pressure compressed the P-frames, reproduction was the first thing dropped. It was never in the I-frame. It was never protected from drift.

The standard explanations aren't wrong. They're downstream. Housing is expensive because the entire society optimized for one dimension and never re-encoded livability. Work culture is brutal because the I-frame says sacrifice, and every institutional delta since 1960 has been faithful to that reference. Education is a pressure cooker because the checkpoint rewards competition, not formation. These aren't independent causes. They're consequences. The root cause is what the I-frame encodes and what it doesn't.
...
If it doesn't track — if I-frame content has no predictive power over which outcomes survive drift — the framework is wrong. Not "needs refinement." Wrong. The checkpoint would be decorative rather than load-bearing, and the convergent structure in the tables above would be coincidence rather than consequence.
