...
Every ad platform has this filter. One label per video, one blocklist per advertiser. The real constraint is per-publisher: a mental health blog needs to exclude gambling with high precision because a single bad ad erodes years of trust, but therapy services and financial literacy should get through. A kids' learning site needs tight filtering on audience, loose on service type.

Lexical filtering can't express this — "gambling" blocks the casino resort's restaurant ad, "debt" blocks the financial literacy course. Semantic filtering could know the difference, but in high-dimensional space, regions are expensive to define and evaluate at scale.
...
Each weight starts at 1 and drifts where signal concentrates. Three safeguards keep M honest: regularization toward I, update caps per impression, and daily decay toward 1. [ITML](https://jmlr.org/papers/v8/davis07a.html) (Davis et al., 2007) does this: online, KL-regularized toward I, O(d) per update when diagonal. Start diagonal. Earn the complexity.
